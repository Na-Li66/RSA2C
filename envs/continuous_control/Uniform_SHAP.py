"""Shared uniform-SHAP ablation for Pendulum-v1, BipedalWalker-v3, and Ant-v5."""
...
class ALDDictionary:
    ...
    def prune_to_indices(self, keep_idx_np):
        keep_idx = torch.as_tensor(keep_idx_np, self.device, dtype=torch.long)
...
class KernelValue:
    ...
    # 方案B：MC + kernel ridge（梯度法）
    def fit_mc_ridge_step(self, S_np, G_np, lr=1e-3, steps=1):
        if self.M == 0:
            self.coef = torch.zeros((0,), device=self.device, dtype=DTYPE); return
        S_n = self._norm_batch_t(S_np)
        K = rbf_kernel_mat_torch(S_n, self.centers, self.ell)   # [N,M]
        y = t(G_np, self.device).reshape(-1)                     # [N]
        w = self.coef.clone()
        N = K.shape[0]
        if N == 0:
            return
        for _ in range(max(1, int(steps))):
            Kw = K @ w                   # [N]
            residual = Kw - y            # [N]
            grad = (K.T @ residual) / max(N,1) + self.reg * w    # [M]
            w = w - lr * grad
...
class AdvantageKernelRegressor:
    ...
    拟合 A_t ~ <W, k(s_t) r_t^T>_F  = (W r_t)^T k(s_t)
      r_t = (u_t - mu_t)/sigma^2 (pre-tanh score), k(s_t)∈R^M

    loss = 0.5*(A_pred - A_target)^2 + 0.5*lambda*||W||_F^2
    grad_W = (A_pred - y) * k(s) r^T + lambda*W
    """
    ...
    def fit_batch(self, S_np, U_np, MU_np, A_target_np, lr=1e-2, clip_grad=None, scale_loss=True):
        ...
        # gW = sum_i residual_i * k_i * r_i^T + reg * W
...
def run_one_seed(args, seed, device):
    ...
    use_natgrad = bool(getattr(args, "use_natgrad", 0))
    use_adv_reg = bool(getattr(args, "use_adv_reg", 1))
    adv_mode_str = "ExplicitAdvReg" if use_adv_reg else "RawAdv"
    pg_mode_str = "NatGrad(tanh)" if use_natgrad else "vanilla(tanh)"
    ...
    shap_history = []          # === NEW: 用于缓存每次 SHAP 的 (epoch, phi)
    phi_ema_vec = None

    value_mode_str = "MC-Ridge(G)" if args.value_mode.lower() == "mc_ridge" else "TD(λ)"

    if seed == args.seed:
        tr_str = f" + KL-TR(pg_tr_kl={args.pg_tr_kl})" if args.pg_tr_kl > 0 else ""
        ...
        print(f"[Info] env={args.env} | horizon={args.horizon} | ep/update={args.episodes_per_update} "
              ...
              f"| base_ell={args.actor_ell} | Value={value_mode_str} | Adv={adv_mode_str} "
              f"| PG={pg_mode_str}{tr_str} | SHAP={args.shap_backend.upper()} "
    ...
    for ep in trange(args.epochs+1):
        ...
        # ===== Value Critic：MC-Ridge 或 TD(λ) =====
        if args.value_mode.lower() == "mc_ridge":
            v_critic.fit_mc_ridge_step(S, G, lr=args.lr_v, steps=args.ts_critic_steps)
        else:
        ...
        if args.shap_enable and (ep % max(1, args.shap_interval) == 0) and (ep >= args.shap_warmup):
            ...
            if args.shap_backend == "cme":
                phi_avg = shap_est.phi_for_batch_exact_fast(
                    tg, bg, n_perms=args.shap_perms,
                    sync_perm=bool(args.shap_sync_perm),
                    work_dtype=work_dtype
                )
            else:
                phi_avg = shap_est.phi_for_batch_fast(
                    tg, bg, n_perms=args.shap_perms,
                    sync_perm=bool(args.shap_sync_perm),
                    work_dtype=work_dtype
                )
            ...
            # === NEW: 把本次 SHAP 结果缓存下来，之后统一保存到 npz ===
            if args.save_shap:
                shap_history.append((ep, phi_to_apply.copy()))
        ...
        g_norm = float(torch.linalg.norm(g).item()) if g.numel() else 0.0
        ...
        if use_natgrad and actor.alpha.numel() > 0 and len(S) > 0:
            # 对角 Fisher: F_diag[m,d] = E[(k_m(s)*r_d)^2]
            with torch.no_grad():
                J = K_s.unsqueeze(2) * Rscore.unsqueeze(1)   # [N,M,ad]
                F_diag = (J * J).mean(dim=0)                # [M,ad]
                F_diag = torch.nan_to_num(F_diag, nan=0.0, posinf=0.0, neginf=0.0)
                F_diag = F_diag + float(args.ng_damping)
                Fmean = float(F_diag.mean().item())
                Fmax  = float(F_diag.max().item()) if F_diag.numel() > 0 else 0.0

            g_nat = g / F_diag

            # 先用 g_nat 估计一步 KL，再按 TR 缩放
            if args.pg_tr_kl > 0.0:
                with torch.no_grad():
                    step_mat = args.lr_actor * g_nat
                    mu_old = K_s @ actor.alpha
                    mu_new = K_s @ (actor.alpha + step_mat)
                    diff = mu_new - mu_old
                    kl_pg_est = float((diff.pow(2).mean()) / (2.0 * sigma2))
                    if not np.isfinite(kl_pg_est):
                        kl_pg_est = 0.0
                    if kl_pg_est > args.pg_tr_kl:
                        scale_tr = math.sqrt(max(1e-12, args.pg_tr_kl) / kl_pg_est)
                    else:
                        scale_tr = 1.0
            else:
                g_nat = g / F_diag
            ...
            lr_step, step_norm = actor.update_vanilla(
                ...
                scale_mult=args.pg_scale * scale_tr
            )

            # NG 调试输出
            if ep % max(1, getattr(args, "ng_log_interval", 50)) == 0 and seed == args.seed:
                print(f"[NG-Debug] ep={ep} | ||g||={g_norm:.3g} | Fmean={Fmean:.4g} | "
                      f"Fmax={Fmax:.4g} | step_norm={step_norm:.3g} | kl_est≈{kl_pg_est:.4g} | scale_tr={scale_tr:.3g}")

        else:
            # 普通 KL-TR（原方案）
            if args.pg_tr_kl > 0.0 and actor.alpha.numel() > 0 and len(S) > 0:
                with torch.no_grad():
                    mu_old = (K_s @ actor.alpha)                   # [N,ad]
                    step_mat = g * args.lr_actor                   # delta alpha (before TR)
                    mu_new = (K_s @ (actor.alpha + step_mat))
                    diff = mu_new - mu_old
                    kl_pg_est = float((diff.pow(2).mean()) / (2.0 * sigma2))
                    if not np.isfinite(kl_pg_est):
        ...
        if (ep % args.eval_interval == 0) or (ep == 0):
            eval_ret, succ_rate = evaluate_policy(
                env_ev, actor, args.eval_noise,
            ...
            kl_str = f" | KL_pg≈{kl_pg_est:.3g}" if args.pg_tr_kl>0 or use_natgrad else ""
            F_str  = f" | Fmean={Fmean:.3g}" if use_natgrad else ""
            print(f"{ep:04d} {marker}{shap_note} | eval={eval_ret:.2f} | succ={succ_rate:.2f} "
                  ...
                  f"| sigma={actor.sigma:.3g} | ||alpha||={alpha_norm:.3g} | ||W||={Wnorm:.3g}"
                  f"{kl_str}{F_str} | ||g||={g_norm:.3g} | AdvMode={'NG' if use_natgrad else 'Vanilla'} | device={device}")
    ...
    # === NEW: 训练结束后把 SHAP（以及可选 ell_eff）保存到 npz ===
    if args.save_shap and len(shap_history) > 0:
        os.makedirs(args.shap_out_dir, exist_ok=True)
        shap_epochs = np.array([h[0] for h in shap_history], dtype=np.int32)
        shap_values = np.stack([h[1] for h in shap_history], axis=0)  # [num_records, d_state]

        shap_file = os.path.join(
            args.shap_out_dir,
            f"shap_{args.shap_out_tag}_env-{args.env}_backend-{args.shap_backend}_seed-{seed}.npz"
        )
        ...
        if args.save_ell_eff:
            save_dict["ell_eff"] = actor.ell_eff.detach().cpu().numpy()
            save_dict["base_ell"] = np.asarray(base_ell, dtype=np.float64)

        np.savez(shap_file, **save_dict)
        if seed == args.seed:
            print(f"[SHAP-Save] saved SHAP logs to {shap_file}")
...
def main():
    ...
    p.add_argument("--actor_ell", type=float, default=3.0)

    p.add_argument("--centers_init_a", type=int, default=192)
    p.add_argument("--max_centers_a", type=int, default=4096)
    ...
    p.add_argument("--max_centers_v", type=int, default=30720)
    ...
    p.add_argument("--lam_adv", type=float, default=3e-3)   # 占位
    p.add_argument("--ema_adv", type=float, default=0.97)   # 占位
    ...
    p.add_argument("--pg_scale", type=float, default=2.0,
                   help="对 PG 梯度做整体乘法缩放。")
    p.add_argument("--pg_tr_kl", type=float, default=0.0,
                   help="若 >0，则对策略更新施加 KL-based trust region；0 表示关闭。")

    # NatGrad 相关
    p.add_argument("--use_natgrad", type=int, default=0,
                   help="1: 使用对角近似自然梯度；0: vanilla PG。")
    p.add_argument("--ng_damping", type=float, default=1e-3,
                   help="自然梯度 F 对角上的阻尼项。")
    p.add_argument("--ng_log_interval", type=int, default=50,
                   help="每多少个 epoch 打一条 NG-Debug。")

    # Value 模式：方案B 默认用 MC-Ridge
    p.add_argument("--value_mode", type=str, choices=["td", "mc_ridge"],
                   default="mc_ridge",
                   help="td: 半梯度 TD(λ)；mc_ridge: G_t 的核岭回归（方案B推荐）")
    p.add_argument("--lr_v", type=float, default=2.5e-3)
    p.add_argument("--td_lambda", type=float, default=0.90)

    # Advantage 显式核回归器超参
    p.add_argument("--use_adv_reg", type=int, default=1,
                   help="1: 使用显式 Advantage 核回归器；0: 直接用 A_hat (RawAdv)。")
    ...
    out_csv_path = os.path.join(
        args.out_dir,
        f"eval_return_seed{args.seed}_{args.shap_backend}_{args.shap_enable}_{args.noise}_valuemode-{args.value_mode}.csv"
    )
    ...
    with open(out_csv_path, "w", newline="") as f:
        ...
        w.writerow(header)
        for t_i in range(T):
            row = [t_i, float(mean_curve[t_i]), float(std_curve[t_i])] + \
                  [float(all_curves[i,t_i]) for i in range(len(seeds))]
            w.writerow(row)
    ...
    value_suffix = f" + {args.value_mode.upper()}"
    plt.title(f"{args.env} | mean over {len(seeds)} seeds (explicit Adv kernel reg + tanh-squash{title_suffix}{value_suffix})")
    plt.legend(); plt.tight_layout()
    plt.savefig(args.out_png, dpi=150)
    print(f"[Done] saved CSV to {out_csv_path}, figure to {args.out_png} | "
          f"time={time.time()-t0:.1f}s | device={device}")
